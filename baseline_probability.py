...
baseline = open("./baseline/Baseline-eval-Pisa-new-test", 'w')
couples = open("./baseline/PoI-pairs-test", 'w')
entropy = open("./baseline/Entropy-outer-links-test", 'w')
...
def load_frequencies():
	...
	for k, v in frequencies.iteritems():
		...
		for k1, v1 in v.iteritems():
			...
			print v[k1]
			entropy_k += v[k1]*math.log(v[k1],2)
			couples.write(str(k) + "\t" + str(k1) + "\t" +str(v1) + "\n")
		print entropy_k
		if entropy_k == 0:
			entropy.write(str(k) + "\t" + str (0.00) + "\n")
		else:
			entropy.write(str(k) + "\t" + str (-entropy_k) + "\n")
		print
	couples.close()
	entropy.close()
